...
```typescript
// ❌ v2 支持 :variable 和 {variable} 两种写法，且 .params 会自动编码参数
request
  .get("/cats/{name}/:age") // [!code --]
  .params("name", "fluffy.cat") // [!code --]
  .params("age", 3)

// ✅ v5 仅支持 {variable} 写法，且 .params 不会自动编码参数
request
  .get("/cats/{name}/{age}")  // [!code ++]
  .params("name", encodeURIComponent("fluffy.cat")) // [!code ++]
  .params("age", 3)
```
