...
相比旧版本，`@keq-request/cli` 将类型声明文件和请求函数文件区分存放，
并且增加 `.fn.ts`、`.schema.ts`、`.type.ts` 后缀以区分不同文件类型。
新的代码结构如下：
...
2. 修改 `.keqrc.(yml|json|js|ts)` 配置
    ...
    import { OverwriteOperationIdPlugin, OverwriteQueryOptionsPlugin } from "@keq-request/cli/plugins";                     // [!code ++]
...
Keq v2 仅支持 NodeJS 18+ 和主流现代浏览器，但未明确列出具体的浏览器版本；
Keq v5 则补充了详细的浏览器兼容性列表：
...
const body = await request
  .get<any>("/cats") // 若不显示指定类型，返回类型默认为 Keq<unknown> [!code ++]
```
