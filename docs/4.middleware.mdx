...
| `context.orchestration` | 中间件编排器，提供中间件执行状态和 fork 能力，详见[进阶 - Context 类型体系](/docs/advanced/context-types#orchestration-属性) |
...
:::tip
关于 Context 的类型体系（`KeqContext`、`KeqSharedContext`、`KeqExecutionContext`）和 `context.orchestration` 的详细说明，请参考[进阶 - Context 类型体系](/docs/advanced/context-types)。
:::
