...
:::caution 重试执行范围
每次重试会**重新执行整个中间件管线**，而非仅重新发起 fetch 请求。如果你的中间件有副作用（如日志记录、计数器递增、Token 刷新等），需要通过 `context.data.retry.attempt` 判断当前是否为重试请求：
...
const logMiddleware: KeqMiddleware = async (context, next) => {
  ...
  if (attempt === 0) {
    // 仅在首次请求时执行
    console.log("发起请求:", context.request.__url__.href)
  } else {
