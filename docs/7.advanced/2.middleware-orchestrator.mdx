...
| `context.orchestration.merge(source)` | `Function` | 将分支执行的结果合并回当前上下文 |
...
将一个已完成的分支执行器的结果（`response`、`res`、`output` 等）合并回当前上下文。通常与 `fork()` 配合使用，当你需要在后台执行请求，然后将结果同步回主链路时非常有用：
...
const backgroundRefresh: KeqMiddleware = async (context: KeqExecutionContext, next) => {
  ...
  // 此时 context.response 已更新为分支执行的结果
}
```
