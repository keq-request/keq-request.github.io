...
将 `context.artifacts` 写入磁盘，并将 `context.matcher` 写入 `.keqfilter` 文件。
...
| `download`        | 自定义文档下载逻辑       | `AsyncSeriesBailHook` | `[address: Address, module: ModuleDefinition, task: TaskWrapper]` |
| `afterDownload`   | Download 阶段完成后      | `AsyncSeriesHook`     | `[TaskWrapper]` |
| `beforeValidate`  | 文档 JSON 解析后、校验前 | `AsyncSeriesHook`     | `[object, ModuleDefinition]` |
