...
Keq 提供了类似 NestJS `MiddlewareConsumer` 的编程式接口，支持**按路由注册中间件**以及**排除特定路由**。中间件注册分为两种模式：全局中间件和模块级中间件。
...
### 模块级中间件 — @InjectKeqConsumer()

当中间件只需要对**特定模块**（通常是由 CLI 生成的模块）的请求生效时，使用 `@InjectKeqConsumer()` 装饰器注入一个绑定到该模块的 `KeqConsumer<T>`：
...
:::tip 模块级路由
如需限定中间件仅对特定模块生效，请使用 `@InjectKeqConsumer(ModuleClass)` 注入 `KeqConsumer<T>`，而非在 `forRoutes()` 中传入模块类。
:::
...
:::tip 中间件执行顺序
请求发送时，中间件的执行顺序为：全局中间件 → `register()` 中的模块级中间件 → 通过 `KeqConsumer` 注册的中间件。
:::
...
### 使用 @InjectKeqConsumer 为生成模块注册中间件

除了通过 `register()` 的 `middlewares` 选项配置模块级中间件外，还可以使用 `@InjectKeqConsumer()` 在模块外部动态注册：
...
| 方式 | 适用场景 |
|------|---------|
| `register({ middlewares: [...] })` | 模块注册时静态配置，中间件在模块初始化时即生效 |
| `@InjectKeqConsumer()` | 在其他 Service 中动态注册，支持依赖注入 NestJS Provider |
