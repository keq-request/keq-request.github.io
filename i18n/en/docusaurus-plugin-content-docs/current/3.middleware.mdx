...
Through `request.apply(middleware).forRoutes({ host: 'example.com' })`, the `Middleware` will only take effect when the request domain is `"example.com"`.
...
request
  .apply(responseValidator())
  .forRoutes({ host: 'example.com' }, { pathname: '/api/**' })
```

`.apply()` will match all requests that meet the rules. If we want individual requests not to run `alert`, we can add `option` to `Middleware`:
...
`request.apply(...middlewares)` returns a chainable configuration object for route-scoped middleware registration:

| **Method**                          | **Description**                                                                                     |
| :---------------------------------- | :-------------------------------------------------------------------------------------------------- |
| `.apply(...middlewares)`            | Specify one or more middlewares to apply, returns a config proxy                                    |
| `.exclude(...routes)`               | Exclude routes from matching (OR logic among exclusions)                                            |
| `.forRoutes(...routes)`             | Specify which routes the middleware applies to (OR logic among routes). Returns `request` for chaining |
| `.forAllRoutes()`                   | Apply middleware to all routes (only exclusions take effect). Returns `request` for chaining         |

**`KeqRoutePattern` fields** (AND logic within a single pattern):
...
request
  ...
  .apply(cacheMiddleware).forRoutes({ host: 'cdn.example.com' })
