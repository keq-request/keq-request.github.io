...
<ExecPackageCommandCode command="keq-cli build" />
...
| skipIndex          | false        | `false`                                    | Whether to skip generating `index.ts` files, useful to avoid merge conflicts in team collaboration |
...
The `install-skill` command installs a [Claude Code](https://docs.anthropic.com/en/docs/claude-code) skill file to the current project.

<ExecPackageCommandCode command="keq-cli install-skill" />

After execution, this command copies the built-in skill definition file to `.claude/skills/keq-cli/SKILL.md` in the project directory.

The skill file enables Claude Code to automatically detect the `keq-cli` configuration in the project and intelligently use `keq-cli` generated functions when writing HTTP request code, instead of manually writing raw HTTP requests.

:::tip Recommendation
After initializing `keq-cli` in a new project, it is recommended to run this command to install the skill file for a better AI-assisted development experience.
:::
