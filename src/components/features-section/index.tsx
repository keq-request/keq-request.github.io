    ...
    <section className="relative py-20 sm:py-32 bg-gradient-to-b from-white via-orange-50/30 to-white dark:from-slate-950 dark:via-slate-900 dark:to-slate-950">
      <div className="relative mx-auto max-w-7xl px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 lg:gap-8">
          <FeatureCard
            name="强大的中间件系统"
            description="受 Koa 启发的洋葱模型中间件，让你能够灵活地扩展请求功能。内置重试、超时、并发控制等常用中间件。"
            Icon={IconLayersLinked}
            gradient="from-orange-500 to-amber-500"
          />
          <FeatureCard
            name="优雅的链式调用"
            description="受 SuperAgent 启发的 API 设计，支持链式调用。代码简洁易读，让 HTTP 请求变得优雅而直观。"
            Icon={IconLink}
            gradient="from-yellow-500 to-orange-500"
          />
          <FeatureCard
            name="完备的 TypeScript 支持"
            description="原生 TypeScript 编写，提供完整的类型定义。支持从 Swagger 文档生成类型安全的请求代码。"
            Icon={IconBrandTypescript}
            gradient="from-amber-500 to-yellow-500"
          />
          <FeatureCard
            name="跨平台运行"
            description="同时支持浏览器和 Node.js 环境，一套代码多端运行。基于标准的 Fetch API，兼容性好。"
            Icon={IconWorld}
            gradient="from-orange-400 to-yellow-400"
          />
          <FeatureCard
            name="智能路由匹配"
            description="支持按域名、路径、方法等条件灵活配置中间件。轻松为不同的 API 服务配置不同的处理逻辑。"
            Icon={IconTarget}
            gradient="from-yellow-400 to-amber-400"
          />
          <FeatureCard
            name="开箱即用"
            description="零配置即可使用，同时提供丰富的配置选项。自动处理请求序列化和响应解析。"
            Icon={IconSparkles}
